...
User::User(): _pseudo(""), _hostname(""), _user(""), _server(""), _welcomed(false), _pfd(NULL), _addr(NULL)
{}

User::User(struct pollfd *pfd, struct sockaddr_storage *addr): _pseudo(""), _hostname(""), _user(""), _server(""), _message(""), _welcomed(false), _pfd(pfd), _addr(addr)
{}

User::~User()
{}

void	User::setPseudo(std::string pseudo) {_pseudo = pseudo;}
...
std::string		User::getPseudo(void) const {return (_pseudo);}
...
{
	o << "User :" << std::endl;
	o << "\t- pseudo ->            " << user.getPseudo() << std::endl;
